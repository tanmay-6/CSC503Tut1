...
int main(int argc, char **argv) {
    ...
    servaddr.sin_addr.s_addr = inet_addr("127.0.0.1");  // connect to server on localhost or change
    ...
    // Prepare the new filename path in client_received_files directory
    ...
    // Now run diff utility to check if files are same
    char diff_command[2048];
    // Assuming server file exists in current directory
